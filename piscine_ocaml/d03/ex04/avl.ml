...
let right_rotate z =
    ...
    Node (y_val, x, Node (z_val, y_l, z_r))
...
let build_rebalanced_node z value =
    let y = left_or_right z value in
    let x = left_or_right y value in
    if is_on_left z y && is_on_left y x then right_rotate z
    else if is_on_left z y && is_on_left y x = false then let new_tree
        = left_rotate y in right_rotate (Node (get_val z, new_tree, get_right
        z))
...
let rec rebalance tree value =
    match tree with
        | Nil -> Nil
        | Node (v, l, r) -> 
                if is_balanced l = false then Node (v, rebalance l value, r)
                else if is_balanced r = false then Node (v, l, rebalance r value)
                else
                    build_rebalanced_node tree value
