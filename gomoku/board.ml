...
let make_board n =
	Array.init n (fun i -> Array.make n Tile.Empty)
...
let place_tile board y x is_red =
	...
	let _make_capture y_decal x_decal acc =
		if (board.(y + (y_decal * 1)).(x + (x_decal * 1)) = other_tile &&
			board.(y + (y_decal * 2)).(x + (x_decal * 2)) = other_tile &&
			board.(y + (y_decal * 1)).(x + (x_decal * 1)) = tile) then (
...
let can_place_tile board y x is_red =
	let tile = if is_red then Tile.Red else Tile.Blue in
	let other_tile = if is_red then Tile.Blue else Tile.Red in
	let _is_empty () =
	...
	let _no_double_threes () =
		let rec _cancel_captures = function
			| [] -> ()
			| h::tl -> (match h with
				| (decal_y, decal_x) -> board.(y + (decal_y * 1)).(x + (decal_x * 1)) <- other_tile ;
									 board.(y + (decal_y * 2)).(x + (decal_x * 2)) <- other_tile
			) ; _cancel_captures tl
		in 
		let _in_list lst elt =
			List.exists ((=) elt) lst
		in
		let ori_threes = free_threes board y x tile in
		let capt = place_tile board y x is_red in
		let new_threes = free_threes board y x tile in
		let diff = List.filter (fun elt -> not (_in_list ori_threes elt)) new_threes in
		_cancel_captures capt ;
		board.(y).(x) <- Tile.Empty ;
		List.length diff < 2 
	in
	_is_empty () && _no_double_threes ()

let print_board board =
	let _print_row row =
		Array.iter (fun x -> Tile.print_tile x ; print_string " ") row ;
		print_endline "" ;
	in
	Array.iter (_print_row) board
...
let valid_moves board is_red =
	let rec _make_columns acc y upto = function
		...
		| i -> (
				...
				let acc = if can_place_tile board y i is_red then (y, i)::acc else acc in
...
let () =
	...
	ignore (place_tile board 1 1 true) ;
	ignore (place_tile board 2 2 true) ;
	ignore (place_tile board 4 5 true) ;
	ignore (place_tile board 4 6 true) ;
	print_board board ;
	for i = 0 to 1000000 do
		ignore (can_place_tile board 4 4 true)
	done ;
	print_board board ;
	let moves = valid_moves board true in
	print_moves moves
