...
/*   Updated: 2016/01/05 17:26:02 by dbousque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void	calculate_moves_score2(t_pile *a, t_pile *b, int scores[11], int i)
{
	if (i == 5 && nb_elts(a) > 2)
		...
		scores[i] = apply_function_and_cancel(a, b, rotate_ab,
														reverse_rotate_ab);
		...
		scores[i] = apply_function_and_cancel(a, b, reverse_rotate_ab,
																rotate_ab);
}
...
void	calculate_moves_score(t_pile *a, t_pile *b, int scores[11])
{
	...
	while (i < 11)
	{
		...
		else if (i == 1 && nb_elts(b) > 1)
		{
			...
			calculate_moves_score2(a, b, scores, i);
		i++;
	}
}
