...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/08 15:54:17 by dbousque          #+#    #+#             */
/*   Updated: 2016/12/08 16:01:16 by dbousque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*close_and_ret(DIR *dir, char *ret)
{
	closedir(dir);
	return (ret);
}
		...
		if (ft_strcmp(file->d_name, name) == 0)
		{
			...
			if (fd != -1 && fstat(fd, &file_stat) != -1)
			{
				...
				if (!S_ISDIR(file_stat.st_mode))
					return (close_and_ret(dir, tmp_full_path));
			}
			free(tmp_full_path);
		}
	}
	return (close_and_ret(dir, NULL));
}
